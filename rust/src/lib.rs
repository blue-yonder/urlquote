...
use percent_encoding::{percent_encode, DEFAULT_ENCODE_SET, percent_decode};
use std::slice;
...
pub unsafe extern "C" fn quote(
    ...
    for (index, byte) in percent_encode(input, DEFAULT_ENCODE_SET)
        .flat_map(str::bytes)
        .enumerate()
        .take(output_len)
    {
...
pub unsafe extern "C" fn unquote(
    ...
    for (index, byte) in percent_decode(input)
        .enumerate()
        .take(output_len)
    {
        output[index] = byte
...
mod tests {
    ...
    fn quoting_works() {
        ...
        unsafe {
            ...
            quote(input.as_bytes().as_ptr(), input.len(), buf.as_mut_ptr(), buf.len());
    ...
    fn unquoting_works() {
        ...
        unsafe {
            ...
            let mut buf = vec![0; buf_len];
            unquote(input.as_bytes().as_ptr(), input.len(), buf.as_mut_ptr(), buf.len());
